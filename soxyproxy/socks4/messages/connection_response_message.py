...
from soxyproxy.server import ResponseMessage
...
class Socks4ConnectionResponseMessage(ResponseMessage):
    ...
    def as_bytes(self):
        port_bytes = int.to_bytes(
            self.port, SOCKS4_ADDRESS_PORT_BYTES_LENGTH, SOCKS4_ADDRESS_PORT_BYTES_ORDER
        )

        return (
            bytes([SOCKS4_REPLY_VERSION, self.reply.value])
            + port_bytes
            + self.address.packed
        )
